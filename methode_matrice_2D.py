...
def methode_matrice_2D_b(planete, p, l_x, l_z, Lx, Lz, temps, d , sparse = False):
    ...
    for i in np.arange(1,Nz+1,1): #i=1,..,Nz - numérotation des nœuds sur un maillage physique
       ...
       for j in np.arange(1,Nx+1,1): #j=1,..,Nx - numérotation des nœuds sur un maillage physique
          ...
          #    b[Aindex(i,j)] = 294           
          #  else :
              # print('indice ne rentre dans aucune catégorie b')
...
A, b, Nx, Nz = methode_matrice_2D(planets_constants['earth'], p=p, l_x=l_x, l_z=l_z, Lx=Lx, Lz=Lz, temps = temps, d =d)

A = methode_matrice_2D_A(planets_constants['earth'], p=p, l_x=l_x, l_z=l_z, Lx=Lx, Lz=Lz, temps = temps, d =d)
