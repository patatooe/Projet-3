...
def methode_matrice_2D_temporelle (planete, p, l_x, l_z, Lx, Lz, d ):
    ...
    abri = False # Est-ce qu'il y a un abri

    A = methode_matrice_2D_A(planete, p, l_x, l_z, Lx, Lz, d, abri=abri)
...
--------------------------------------------------------------------------------------------------------
    ...
    for t in tqdm(np.arange(dt, temps_deval, dt), total=nb_iterations):
        ...
        b_prime_1 = (M + dt*(1-xi)/(alpha*d**2)*A).dot(Un)
